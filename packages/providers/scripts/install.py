...
class SliceInstaller:
    ...
    def check_system_requirements(self) -> bool:
        ...
        if python_version < (3, 10):
            self.log(
                f"Python {python_version.major}.{python_version.minor} não suportado. Mínimo: 3.10",
                "ERROR",
            )
            return False
        ...
        try:
            ...
            if free_gb < 10:  # Mínimo 10GB livres
                self.log(
                    f"Espaço insuficiente: {free_gb}GB livres. Mínimo: 10GB", "ERROR"
                )
                return False
    ...
    def setup_python_environment(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                subprocess.run(
                    [sys.executable, "-m", "pip", "install", "pdm"], check=True
                )
    ...
    def show_success_summary(self) -> None:
        ...
        print(
            f"{self.BLUE}📦 Etapas completadas: {len(self.steps_completed)}{self.RESET}"
        )

        print(f"\n{self.BOLD}🚀 PRÓXIMOS PASSOS:{self.RESET}")
        print(
            f"{self.GREEN}1. task dev{self.RESET}     # Inicia servidor em desenvolvimento"
        )
        print(
            f"{self.GREEN}2. http://localhost:{SERVER_PORT}/docs{self.RESET} # Documentação da API"
        )
    ...
    def run(self) -> None:
        ...
        if not self.setup_python_environment():
            self.handle_failure(
                "python_env", "Falha na configuração do ambiente Python"
            )
