...
class SliceProductionServer:
    ...
    def check_production_requirements(self) -> bool:
        ...
        try:
            ...
            self.log(
                f"RAM: {memory.total // (1024**3)}GB total, {memory.available // (1024**3)}GB disponível",
                "INFO",
            )
        ...
        try:
            ...
            for file_path in critical_files:
                if not Path(file_path).exists():
                    ...
                    self.log(
                        "Execute 'task install' antes de iniciar produção", "ERROR"
                    )
                    return False
    ...
    def start_production_server(self) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                str(SERVER_PORT),
                "--workers",
                "1",  # Single worker para economizar recursos
                "--log-level",
                LOG_LEVEL.lower(),
                "--no-access-log",  # Reduz overhead de logging
                "--loop",
                "uvloop",  # Loop otimizado
                "--http",
    ...
    def monitor_production_server(self) -> None:
        ...
        print(
            f"{self.CYAN}❤️  Health: http://{SERVER_HOST}:{SERVER_PORT}/health{self.RESET}"
        )
        print(
            f"{self.CYAN}📚 Docs: http://{SERVER_HOST}:{SERVER_PORT}/docs{self.RESET}"
        )
    ...
    def check_resources(self) -> None:
        ...
        try:
            ...
            if memory.percent < 80 and cpu_percent < 60:
                self.log(
                    f"Recursos OK - RAM: {memory.percent:.1f}%, CPU: {cpu_percent:.1f}%",
                    "INFO",
                )
