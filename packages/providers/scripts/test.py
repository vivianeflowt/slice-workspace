...
class SliceTestRunner:
    ...
    def check_test_environment(self) -> bool:
        ...
        try:
            # Verifica pytest
            result = subprocess.run(["pdm", "run", "pytest", "--version"], 
    ...
    def run_lint_tests(self) -> bool:
        ...
        try:
            ...
            # isort check
            result = subprocess.run(
                ["pdm", "run", "isort", "--check-only", "server/", "tests/", "scripts/"],
                capture_output=True,
                text=True,
                cwd=Path(__file__).parent.parent
            )
    ...
    def generate_report(self) -> None:
        ...
        if self.results["failed"] == 0:
            ...
            print(f"{self.RED}{self.BOLD}❌ {self.results['failed']} TESTE(S) FALHARAM{self.RESET}")
