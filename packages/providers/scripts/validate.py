...
class SliceValidator:
    ...
    def validate_project_structure(self) -> bool:
        ...
        try:
            ...
            self.validation_results["structure"]["details"] = [
    ...
    def validate_dependencies(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["pdm", "--version"], capture_output=True, text=True
            )
            ...
            self.validation_results["dependencies"]["details"] = [
                f"PDM: {result.stdout.strip()}",
                f"Deps críticas: {len(critical_deps)} OK",
            ]
            return True
    ...
    def validate_code_quality(self) -> bool:
        ...
        try:
            ...
            for check in code_checks:
                try:
                    result = subprocess.run(
                        check["cmd"], capture_output=True, text=True, timeout=30
                    )
    ...
    def validate_tests(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.validation_results["tests"]["details"] = [
    ...
    def generate_validation_report(self) -> None:
        ...
        total_validations = len(
            [k for k in self.validation_results.keys() if k != "overall"]
        )
        overall_score = (
        ...
        for category, result in self.validation_results.items():
            ...
            if isinstance(result, dict):
                ...
                print(
                    f"{color}{status}{self.RESET} - {category.replace('_', ' ').title()}"
                )
        ...
        if self.validation_results["overall"]["passed"]:
            ...
            print(
                f"{self.GREEN}Projeto atende a todos os padrões Slice/ALIVE Enterprise{self.RESET}"
            )
        else:
            print(f"\n{self.RED}{self.BOLD}❌ VALIDAÇÃO FALHOU{self.RESET}")
            print(
                f"{self.RED}Score insuficiente: {overall_score:.1%} (mínimo: 80%){self.RESET}"
            )
