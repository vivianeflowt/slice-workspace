...
async def classify_text(
    request: ClassificationRequest, provider=Depends(get_classification_provider)
) -> ClassificationResponse:
    """
    ...
    try:
        ...
        result = provider.classify_text(
            text=request.text, labels=request.labels, top_k=5  # Máximo de 5 predições
        )
...
async def classify_batch(
    request: BatchRequest, provider=Depends(get_classification_provider)
) -> BatchResponse:
    """
    ...
    try:
        if request.function != "classify":
            raise HTTPException(
                status_code=400, detail="Função deve ser 'classify' para este endpoint"
            )
...
async def zero_shot_classify(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro no zero-shot classification: {str(e)}"
        )
