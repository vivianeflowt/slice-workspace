...
from server.constants import MAX_BATCH_SIZE, REQUEST_TIMEOUT_SECONDS
...
def get_embedding_provider(model_name: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno no provider de embeddings: {str(e)}"
        )


@router.post("/embed", response_model=EmbeddingResponse)
async def generate_embedding(
    request: EmbeddingRequest, provider=Depends(get_embedding_provider)
) -> EmbeddingResponse:
    """
    ...
    try:
        result = provider.generate_single_embedding(
            text=request.text, normalize=request.normalize
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro na geração de embedding: {str(e)}"
        )


@router.post("/embed/batch", response_model=BatchResponse)
async def generate_embeddings_batch(
    request: BatchRequest, provider=Depends(get_embedding_provider)
) -> BatchResponse:
    """
    ...
    try:
        if request.function != "embed":
            raise HTTPException(
                status_code=400, detail="Função deve ser 'embed' para este endpoint"
            )

        if len(request.texts) > MAX_BATCH_SIZE:
            raise HTTPException(
                status_code=400, detail=f"Máximo de {MAX_BATCH_SIZE} textos por lote"
            )
        ...
        result = provider.generate_embeddings(
            texts=request.texts, normalize=True  # Padrão normalizado
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro na geração de embeddings em lote: {str(e)}"
        )
...
async def compute_similarity(
    ...
    try:
        result = provider.compute_similarity(
            text1=text1, text2=text2, normalize=normalize
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro no cálculo de similaridade: {str(e)}"
        )


@router.post("/embed/multi", response_model=EmbeddingResponse)
async def generate_multiple_embeddings(
    texts: List[str], normalize: bool = True, provider=Depends(get_embedding_provider)
) -> EmbeddingResponse:
    """
    ...
    try:
        if len(texts) > MAX_BATCH_SIZE:
            raise HTTPException(
                status_code=400, detail=f"Máximo de {MAX_BATCH_SIZE} textos por vez"
            )

        result = provider.generate_embeddings(texts=texts, normalize=normalize)
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro na geração de múltiplos embeddings: {str(e)}"
        )
...
async def list_embedding_models() -> Dict[str, Any]:
    ...
    from server.constants import ALTERNATIVE_MODELS, DEFAULT_MODELS
