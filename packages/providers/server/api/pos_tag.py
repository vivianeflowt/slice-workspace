...
def get_pos_tag_provider(model_name: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro interno no provider de POS tagging: {str(e)}"
        )


@router.post("/pos-tag", response_model=PosTagResponse)
async def tag_text(
    request: PosTagRequest, provider=Depends(get_pos_tag_provider)
) -> PosTagResponse:
    """
    ...
    try:
        result = provider.tag_text(
            text=request.text, return_confidence=True, aggregate_subwords=True
        )
        ...
        raise HTTPException(status_code=500, detail=f"Erro no POS tagging: {str(e)}")


@router.post("/pos-tag/batch", response_model=BatchResponse)
async def tag_batch(
    request: BatchRequest, provider=Depends(get_pos_tag_provider)
) -> BatchResponse:
    """
    ...
    try:
        if request.function != "pos_tag":
            raise HTTPException(
                status_code=400, detail="Função deve ser 'pos_tag' para este endpoint"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro no POS tagging em lote: {str(e)}"
        )


@router.post("/pos-tag/analyze")
async def analyze_sentence_structure(
    text: str, provider=Depends(get_pos_tag_provider)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro na análise estrutural: {str(e)}"
        )


@router.get("/pos-tag/tags")
async def get_supported_tags(provider=Depends(get_pos_tag_provider)) -> Dict[str, Any]:
    """
...
async def get_pos_tag_info(provider=Depends(get_pos_tag_provider)) -> Dict[str, Any]:
