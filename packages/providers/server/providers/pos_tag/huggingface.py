...
class HuggingFacePosTagProvider:
    ...
    def tag_text(
        self, text: str, return_confidence: bool = True, aggregate_subwords: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            results = self.pipeline(
                text, aggregation_strategy="simple" if aggregate_subwords else "none"
            )

            # Processa resultados
            tokens = self._process_results(
                results, text, return_confidence, aggregate_subwords
            )
    ...
    def analyze_sentence_structure(self, text: str) -> Dict[str, Any]:
        ...
        has_subject = any(
            tag in ["substantivo", "pronome", "nome_próprio"] for tag in tag_counts
        )
